...
void
write_perf_test(avro_schema_t writers_schema, avro_datum_t datum, char *type, int32_t count)
{
	int validate = 0;
...
static int test_string(void)
{
	...
	for (i = 0; i < sizeof(strings) / sizeof(strings[0]); i++) {
		...
		write_perf_test(writer_schema, datum, "string", 1000000);
...
static int test_bytes(void)
{
	...
	write_perf_test(writer_schema, datum, "bytes", 1000000);
	avro_datum_decref(datum);
	avro_schema_decref(writer_schema);
	return 0;
}
...
static int test_int32(void)
{
	...
	for (i = 0; i < 100; i++) {
		...
		write_perf_test(writer_schema, datum, "int", 1000000);
		avro_datum_decref(datum);
	}
	avro_schema_decref(writer_schema);
	return 0;
...
static int test_int64(void)
{
	...
	for (i = 0; i < 100; i++) {
		...
		write_perf_test(writer_schema, datum, "long", 1000000);
		avro_datum_decref(datum);
	}
	avro_schema_decref(writer_schema);
	return 0;
...
static int test_double(void)
{
	...
	for (i = 0; i < 100; i++) {
		...
		write_perf_test(schema, datum, "double", 1000000);
		avro_datum_decref(datum);
	}
...
static int test_float(void)
{
	...
	for (i = 0; i < 100; i++) {
		...
		write_perf_test(schema, datum, "float", 1000000);
		avro_datum_decref(datum);
	}
	avro_schema_decref(schema);
	return 0;
...
static int test_boolean(void)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		write_perf_test(schema, datum, "boolean", 1000000);
		avro_datum_decref(datum);
	}
	avro_schema_decref(schema);
	return 0;
...
static int test_null(void)
{
	...
	write_perf_test(schema, datum, "null", 1000000);
	avro_datum_decref(datum);
	return 0;
}
...
static int test_record(void)
{
	...
	write_perf_test(schema, datum, "record", 10000000);
...
static int test_enum(void)
{
	...
	write_perf_test(schema, datum, "enum", 1000000);
	avro_datum_decref(datum);
	avro_schema_decref(schema);
	return 0;
}
...
static int test_array(void)
{
	...
	write_perf_test(schema, datum, "array", 1000000);
	avro_datum_decref(datum);
	avro_schema_decref(schema);
	return 0;
}
...
static int test_map(void)
{
	...
	write_perf_test(schema, datum, "map", 1000000);
	avro_datum_decref(datum);
	avro_schema_decref(schema);
	return 0;
}
...
static int test_union(void)
{
	...
	write_perf_test(schema, union_datum, "union", 1000000);
...
static int test_fixed(void)
{
	...
	write_perf_test(schema, datum, "fixed", 1000000);
	avro_datum_decref(datum);
	avro_schema_decref(schema);
	return 0;
}
